...
@nb.njit()
def Force_long_cal(POSCAR, KCAR, k_f1, alpha, L_x, L_y, L_z, eps_0, eps_1, eps_2, eps_3, S, z_list, equal_cell, equal_size, equal_cell_room_num):
    ...
    Force_long_z_self = np.zeros(NUM_particle)


    flag = 0
    while KCAR[flag][3] <= k_f1:
        k_p = KCAR[flag]
        k_x, k_y, k_d, k = k_p
        flag += 1
        ...
        for j in range(NUM_particle):
            Force_long_x_k_sum_val[j] += + (1/(L_x * L_y * eps_0 * eps_2)) * (F_x_sum1_val[j] + F_x_sum2_val[j]) * exp(-k_d/(4 * alpha))
            Force_long_y_k_sum_val[j] += + (1/(L_x * L_y * eps_0 * eps_2)) * (F_y_sum1_val[j] + F_y_sum2_val[j]) * exp(-k_d/(4 * alpha))
            # Force_long_z_k_sum_val[j] += + (1/(L_x * L_y * eps_0 * eps_2)) * (F_z_sum1_val[j] - F_z_sum2_val[j] + F_z_self_val[j]) * exp(-k_d/(4 * alpha))
            Force_long_z_k_sum_val[j] += + (1/(L_x * L_y * eps_0 * eps_2)) * (F_z_sum1_val[j] - F_z_sum2_val[j]) * exp(-k_d/(4 * alpha))

            Force_long_z_self[j] += (1/(L_x * L_y * eps_0 * eps_2)) * F_z_self_val[j] * exp(-k_d/(4 * alpha))
    ...
    return Force_long_x, Force_long_y, Force_long_z, Force_long_z_self


def Force_long_self(POSCAR, INCAR, CELL_LIST, KCAR):
    RBM_p = INCAR['RBM_p']
    K_set = INCAR['K_set']
    ...
    Force_long_x, Force_long_y, Force_long_z, Force_long_z_self = Force_long_cal(POSCAR, KCAR, k_f1, alpha, L_x, L_y, L_z, eps_0, eps_1, eps_2, eps_3, S, z_list, equal_cell, equal_size, equal_cell_room_num)

    return Force_long_x, Force_long_y, Force_long_z, Force_long_z_self


def Force_long(POSCAR, INCAR, CELL_LIST, KCAR):
    RBM_p = INCAR['RBM_p']
    K_set = INCAR['K_set']
    K_set_shape = np.shape(K_set)[0]
    L_x = INCAR['L_x']
